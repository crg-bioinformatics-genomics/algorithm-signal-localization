...
echo $2 |  tr [a-z] [A-Z] | awk '(length($1)<750)&&(length($1)>=50){print "'$1'", $1}' > ./database/protein.50.750.txt 
...
bash ./bin/protein-feature.sh    ./database/protein.50.750.txt   > ./database/protein.dat
...
bash ./bin/fourier.line.fast.sh     ./database/protein.750.dat    | awk '(NF==53)&&($2!=0)' 
