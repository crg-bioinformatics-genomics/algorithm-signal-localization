...
cat $f/min.relative.i.$model.txt $f/max.relative.i.$model.txt  input.raw.tmp  |   awk '(NR==1){for(i=1;i<=NF;i++){m[i]=$i}} (NR==2){for(i=1;i<=NF;i++){M[i]=$i}} (NR>2)&&(NR%2!=0){for(i=1;i<=NF;i++){ s=0; if (m[i]!=M[i]){s=(($i-m[i])/(M[i]-m[i])-1/2)*2;} printf "%f\t",s} printf "\n"} (NR>2)&&(NR%2==0){print $0}'  > tmp/norm.tmp
